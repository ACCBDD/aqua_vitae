...
public class SwayCamera {
    ...
    public static void onComputeCameraAngles(ViewportEvent.ComputeCameraAngles event) {
        ...
        if (!Minecraft.getInstance().isPaused() && player.getData(ModAttachments.BLOOD_ALCOHOL) > 0) {
            float targetFactor = player.getData(ModAttachments.BLOOD_ALCOHOL) / 30000f * Minecraft.getInstance().options.screenEffectScale().get().floatValue();
